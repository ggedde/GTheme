...
class GBLOCKS_PLUGIN_SETTINGS
{
	...
	{
		if(!empty($_POST['save_gblock_settings']) && !empty($_POST['settings']) && check_admin_referer(self::$option_key))
		{
			...
			$settings['gblock_settings_last_updated'] = time();
	...
	public static function get_form($fields)
	{
		...
		?>
			...
			<style>
				/******* Set Default Styling ******/
				.gblocks-plugin-settings-form .repeater-item {
					border: 1px solid #bbb;
				}
				.gblocks-plugin-settings-form .repeater-item td {
				...
				.gblocks-plugin-settings-field-colorpicker .wp-picker-container {
					min-width: 250px;
				}
				.wp-picker-holder {
					position: absolute;
			...
			<form method="post" class="gblocks-plugin-settings-form">
				<input type="hidden" name="save_gblock_settings" value="1">
				...
				{
					?>
					<tr class="gblocks-plugin-settings-<?php echo $meta_key;?> gblocks-plugin-settings-type-<?php echo $field['type'];?>">
									...
									foreach ($field['fields'] as $rep_i => $rep_fields)
									{
										...
										<tr class="repeater-item<?php echo (!is_numeric($rep_i) ? ' repeater-placeholder' : '');?>" style="z-index:<?php echo (is_numeric($rep_i) ? $rep_i : 0);?>;">
											...
											foreach ($rep_fields as $rep_key => $rep_field)
											{
												...
												{
													?>
													<td class="gblocks-plugin-settings-field gblocks-plugin-settings-field-<?php echo $rep_field['type'];?>">
			...
			jQuery(function($)
			{
				...
				$('.repeater-add').on('click', function(e)
				{
					...
					clone.find('.gblocks-blocks-colorpicker').wpColorPicker();
				...
				addRemoveListeners();
				$('.repeater-item:not(.repeater-placeholder) .gblocks-blocks-colorpicker').wpColorPicker();
	...
	public static function settings_field($meta_key, $field, $repeater_key='', $rep_i=0)
	{
				...
				<span class="gblocks-option-wrapper">
					...
					<?php if($block_description){ ?>
						...
						<div style="display:none;"><div id="<?php echo $option_value; ?>">
							<div class="gblocks-modal-content">
		...
		{
			?><input type="text" class="gblocks-blocks-colorpicker" name="<?php echo $settings_attribute;?>" id="<?php echo $meta_key;?>"<?php echo (isset($field['maxlength']) ? ' maxlength="'.$field['maxlength'].'"' : '');?> value="<?php echo esc_attr( (isset($field['value']) ? $field['value'] : '') );?>" class="regular-text" /><br /><?php
		}
		?>
		</span>
		<?php
